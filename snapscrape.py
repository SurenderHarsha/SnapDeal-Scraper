...
def scrape(a):
    ...
    while 1:
        try:
            cur.execute('create table '+name+'(name text,price text,rating text,image text)')
            break
        except:
            k+=1
    ...
    for i in range(len(rat)):
        j=images[i][0].split()
        if rat[i]==[]:
            cur.execute("insert into "+name+" values(?,?,?,?)",(tname[i][0],pric[i][0],"NULL",j[0][:-1]))
            continue
        cur.execute("insert into "+name+" values(?,?,?,?)",(tname[i][0],pric[i][0],rat[i][0][1:-1],j[0][:-1]))
    db.commit()
    print "Sort By:\n1.Name\n2.Price\n3.Rating\n"
    t=['','name','price','rating']
    p=int(raw_input())
    ...
    file.write('<table>')
    for row in curs:
        file.write('<tr><td>'+row[0]+'</td><td><h3>Rs.'+row[1]+'</h3></td><td>'+row[2]+'')
